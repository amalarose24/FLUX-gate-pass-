...
function FacultyDash() {
    ...
    // Security check: Only advisors or wardens see Analytics and Approvals
    const isAdvisor = !!user?.advisorBatch || user?.role === 'warden';
    const [activeTab, setActiveTab] = useState(isAdvisor ? 'analytics' : 'log');
    ...
    const refreshApprovals = () => {
        ...
        axios.get(`http://localhost:5000/api/pass/approver-data/${user?._id}?page=${page}&limit=50`, { headers: { Authorization: `Bearer ${token}` } }).then(res => {
            ...
            setApprHistory(res.data.history);
            setTotalPages(Math.ceil(res.data.totalHistoryCount / 50) || 1);
        });
        axios.get(`http://localhost:5000/api/analytics?role=${user?.role}&batch=${user?.advisorBatch || user?.batch || ''}&timeframe=${timeframe}`)
    ...
    const decide = (id, status) => {
        ...
        const token = localStorage.getItem('token');
        const payload = { status, approverId: user?._id, rejectReason: status === 'rejected' ? rejectReason : '' };
    ...
    const exportToCSV = () => {
        ...
        const rows = historyToExport.map(req => {
            ...
            return [
                ...
                req.userId?.name || 'Unknown', req.userRole || 'Unknown',
                `"${(req.destination || '').replace(/"/g, '""')}"`,
                `"${(req.reason || '').replace(/"/g, '""')}"`, req.status
            ];
        });
        ...
        link.setAttribute("download", `Faculty_Log.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    if (!user) return <div className="min-h-screen flex items-center justify-center font-bold text-slate-500">Loading User...</div>;
        ...
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
            ...
            <div className="bg-gradient-to-r from-teal-500 to-teal-700 pt-28 pb-20 px-6">
                <div className="max-w-6xl mx-auto">
                    <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight">
                        Welcome back, {user?.name?.startsWith('Prof') ? user?.name : `Prof. ${user?.name?.split(' ')[0] || ''}`}
                    </h1>
            ...
            <div className="max-w-6xl mx-auto px-6 -mt-12 relative z-10 pb-12">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-4 mb-8 bg-white p-2 rounded-2xl shadow-sm w-fit border border-slate-100">
                    {isAdvisor && <button className={`px-5 sm:px-6 py-2 rounded-xl border-none font-semibold text-sm transition-all duration-300 ${activeTab === 'analytics' ? 'bg-teal-500 text-white shadow-md shadow-teal-500/30' : 'text-slate-600 hover:bg-slate-100'}`} onClick={() => setActiveTab('analytics')}>Analytics</button>}
                    {isAdvisor && <button className={`px-5 sm:px-6 py-2 rounded-xl border-none font-semibold text-sm transition-all duration-300 flex items-center gap-2 ${activeTab === 'approve' ? 'bg-teal-500 text-white shadow-md shadow-teal-500/30' : 'text-slate-600 hover:bg-slate-100'}`} onClick={() => setActiveTab('approve')}>Approvals {pending.length > 0 && <span className="bg-rose-500 text-white px-2 py-0.5 rounded-md text-xs font-black shadow-sm">{pending.length}</span>}</button>}
                    <button className={`px-5 sm:px-6 py-2 rounded-xl border-none font-semibold text-sm transition-all duration-300 ${activeTab === 'log' ? 'bg-teal-500 text-white shadow-md shadow-teal-500/30' : 'text-slate-600 hover:bg-slate-100'}`} onClick={() => setActiveTab('log')}>My Movement</button>
                </div>

                {/* ANALYTICS TAB */}
                {activeTab === 'analytics' && analytics && isAdvisor && (
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {/* Pass Status - Faculty Data */}
                            <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
                                <h4 className="text-slate-500 text-center mb-4 text-sm font-bold uppercase tracking-widest">Pass Status</h4>
                                <div className="h-56">
                                    {analytics.batchStatus?.length > 0 ? (
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie data={analytics.batchStatus} cx="50%" cy="50%" innerRadius={55} outerRadius={70} paddingAngle={4} dataKey="value">
                                                    {analytics.batchStatus.map((entry, index) => <Cell key={`cell-${index}`} fill={getStatusChartColor(entry.name, index)} stroke="rgba(0,0,0,0.05)" strokeWidth={1} />)}
                                                </Pie>
                            ...
                            {/* Top Destinations - Faculty Data */}
                            <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
                                <h4 className="text-slate-500 text-center mb-4 text-sm font-bold uppercase tracking-widest">Top Destinations</h4>
                                <div className="h-56">
                                    {analytics.batchDestinations?.length > 0 ? (
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie data={analytics.batchDestinations} cx="50%" cy="50%" innerRadius={30} outerRadius={70} paddingAngle={2} dataKey="value">
                                                    {analytics.batchDestinations.map((entry, index) => <Cell key={`cell-${index}`} fill={GENERAL_COLORS[index % GENERAL_COLORS.length]} stroke="rgba(0,0,0,0.05)" strokeWidth={1} />)}
                                                </Pie>
                            ...
                            {/* Top Requesters - Faculty Data (Bar Chart) */}
                            <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
                                <h4 className="text-slate-500 text-center mb-4 text-sm font-bold uppercase tracking-widest">Top Requesters</h4>
                                <div className="h-56">
                                    {analytics.topRequesters?.length > 0 ? (
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={analytics.topRequesters} layout="vertical" margin={{ top: 0, right: 20, left: 10, bottom: 0 }}>
                                                <XAxis type="number" hide />
                                                <YAxis type="category" dataKey="name" stroke="#94a3b8" fontSize={11} tickLine={false} axisLine={false} width={80} />
                                                <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(0,0,0,0.03)' }} />
                                                <Bar dataKey="value" fill="#818cf8" radius={[0, 4, 4, 0]} barSize={14} />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    ) : <p className="text-slate-400 text-center mt-20 text-sm font-medium">No data available yet.</p>}
                                </div>
                                    ...
                                    {pending.map(req => (
                                        <div key={req._id} className="bg-white p-5 rounded-2xl shadow-md border border-slate-100 border-l-4 border-l-amber-500 hover:shadow-lg transition-shadow">
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                <span className="text-[10px] bg-slate-100 px-2 py-1 rounded text-slate-500 font-bold tracking-wider">STUDENT</span>
                                            </div>
